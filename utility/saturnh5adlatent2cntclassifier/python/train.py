...
if model=='GradientBoostingClassifier': # very slow
	clf=Pipeline([
		...
		('classifier', GradientBoostingClassifier(n_estimators=1024, random_state=seed)),
		])
	...
	clf=Pipeline([
		('scaler', StandardScaler()),
		('classifier', LogisticRegression(multi_class='ovr', random_state=seed))
		])
	...
	clf=Pipeline([
		('scaler', StandardScaler()),
		('nca', NeighborhoodComponentsAnalysis(random_state=seed)),
		('knn', KNeighborsClassifier()),
		])
elif model=='RandomForestClassifier':
	clf=Pipeline([
		('scaler', StandardScaler()),
		('classifier', RandomForestClassifier(n_estimators=1024, random_state=seed)),
		])
elif model=='SGDClassifier':
	clf=Pipeline([
		('scaler', StandardScaler()),
		('classifier', SGDClassifier(loss='log', random_state=seed)),
		])
elif model=='SVC':
	clf=Pipeline([
		('scaler', StandardScaler()),
		('classifier', SVC(kernel='rbf', random_state=seed, probability=True, decision_function_shape='ovr')),
		])
elif model=='SVClinear':
	clf=Pipeline([
		('scaler', StandardScaler()),
		('classifier', SVC(kernel='linear', random_state=seed, probability=True, decision_function_shape='ovr')),
		])
...
tmp={
	...
	# 'auc': roc_auc_score(y, clf.predict_proba(X), average='weighted', multi_class='ovr') if model!='NearestCentroid' else -1,
